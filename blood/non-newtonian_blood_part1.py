...
''' Import Data '''
ViscosityData = np.loadtxt('blood_viscosity_data.txt')
...
''' Plot Data '''
plt.figure(1)
plt.semilogx(ViscosityData[:,0], ViscosityData[:,1], '.', label = 'Rheological data')
# Cross
...
crossL2 = 100*norm(ViscosityData[:,1] - μ_cross(lse_cross.x[0], lse_cross.x[1], 
                                     lse_cross.x[2], lse_cross.x[3],
                                     np.logspace(0,3,len(ViscosityData[:,1]))),2)/ \
                                        norm(ViscosityData[:,1],2)                                        

cassonL1 = 100*norm(ViscosityData[:,1] - μ_casson(lse_casson.x[0], lse_casson.x[1], 
                                    np.logspace(0,3,len(ViscosityData[:,1]))),1)/ \
                                        norm(ViscosityData[:,1],1)
cassonL2 = 100*norm(ViscosityData[:,1] - μ_casson(lse_casson.x[0], lse_casson.x[1], 
                                    np.logspace(0,3,len(ViscosityData[:,1]))),2)/ \
                                        norm(ViscosityData[:,1],2)